...
const tipo_participante = repository.findOne({ id })
...
const tipo_participante = repository.add(tipo_participanteInput)
return res.status(201).send({ message: 'tipo_participante created', data: tipo_participante })
}

function update(req: Request, res: Response) {
req.body.sanitizedInput.id = req.params.id
const tipo_participante = repository.update(req.body.sanitizedInput)
...
const id = req.params.id
const tipo_participante = repository.delete({ id })
