...
export const login = async (req, res) => {
    ...
    const accessToken = jwt.sign({ userId: usuario.id, email: usuario.email }, process.env.JWT_SECRET, { expiresIn: '1h' });
    // Crear el refresh token (expira en 7 días)
    const refreshToken = jwt.sign({ userId: usuario.id, email: usuario.email }, process.env.JWT_SECRET, { expiresIn: '7d' });
