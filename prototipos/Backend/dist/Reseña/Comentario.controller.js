...
async function add(req, res) {
    try {
        ...
        const comentarioInput = new Reseña(input.comentario, new ObjectId(input.usuario));
...
async function update(req, res) {
    try {
        ...
        if (!comentarioExiste) {
            const objectIdReseñaId = new ObjectId(comentarioId);
            const comentarioInput = new Reseña(updatedData.comentario, updatedData.usuario, objectIdReseñaId);
            const nuevoReseña = await repository.add(comentarioInput);
            if (!nuevoReseña) {
                return res.status(500).send({ message: "Error al crear el nuevo comentario." });
            }
            return res.status(201).send({ message: 'Reseña creado con éxito.', data: nuevoReseña });
        }
        const updatedReseña = await repository.update(comentarioId, updatedData);
        if (!updatedReseña) {
            return res.status(500).send({ message: "Error al actualizar el Reseña" });
        }
        return res.status(200).send({ message: 'Reseña actualizado con éxito.', data: updatedReseña });
    }
    catch (error) {
