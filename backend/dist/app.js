...
const usuarios = [
    new Usuario('a02b91bc-3769-4221-beb1-d7a3aeba7dad', 'agusidana04@gmail.com', '123489sdfafgdfjasdhjahsd', 'DNI', 45949176, 'Urquiza 7884', 3413946996, 'Agustin', 'Dana'),
];
...
app.post('/api/usuarios', sanitizeUsuarioInput, (req, res) => {
    ...
    const usuario = new Usuario(input.id, input.email, input.contrasena, input.tipoDoc, input.numDoc, input.direccion, input.telefono, input.nombre, input.apellido, input.fechaNacimiento, input.nombreFantasia, input.descripcion);
...
app.put('/api/usuarios/:id', sanitizeUsuarioInput, (req, res) => {
    const usurioIdx = usuarios.findIndex((usuario) => usuario.id === req.params.id);
    if (usurioIdx === -1) {
        res.status(404).send({ message: 'Usuario no encontrado' });
        return;
    }
