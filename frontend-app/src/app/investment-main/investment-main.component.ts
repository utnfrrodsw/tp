...
export class InvestmentMainComponent {
  configBanks: Bank[] = [];
  ...
  constructor(
    ...
    forkJoin(observables).subscribe(([banks, iccData, dolars]) => {
      ...
      this.configDolar.map(
        (d) => (d.cost = +(this.investment! / d.venta).toFixed(2))
      );

      this.timeoutId = setTimeout(() => {
        this.configICC.map((y) => {
          this.generateTextforICC(y.InvestmentId, this.investment!).subscribe(
            (value) => {
          ...
          this.configBanks.map((x) => {
            x.content = this.textGeneratorService.generateTextForBanks(
              x.Name,
              x.InterestRate
            );
          }),
  ...
  calculateRevenue() {
    this.revenue.push(
      ...this.configBanks.map(
        (x) =>
          +(
            (x.InterestRate / 100 - this.inflation / 100 + 1) *
            this.investment!
          ).toFixed(2)
      )
    );
    this.revenue.push(
      ...this.configICC.map(
        (x) => +(x.Content! / (1 + this.inflation / 100)).toFixed(2)
      )
    );
    this.revenue.push(
      ...this.configDolar.map((x) => +(x.cost! * x.compra).toFixed(2))
    );
    this.revenue.push(
      +(this.investment! / (1 + this.inflation! / 100)).toFixed(2)
    );
    this.titles.push(...this.configBanks.map((x) => x.Name.toLowerCase()));
    this.titles.push(...this.configICC.map((x) => x.Label.toLowerCase()));
    this.titles.push(
      ...this.configDolar.map((x) => 'dolar ' + x.nombre.toLowerCase())
    );
